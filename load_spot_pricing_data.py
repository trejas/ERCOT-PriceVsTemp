...
def get_data_filenames(years):
    ...
    for year in years:
        path = r'.\data\spot_pricing\{}'.format(year)
...
def load_data(years):
    ...
    spot_pricing_df = pd.concat(dfs, ignore_index=True)

    print("Spot Prices Loaded")

    return spot_pricing_df


spot_pricing_df = load_data([2013, 2014, 2015, 2016, 2017])
